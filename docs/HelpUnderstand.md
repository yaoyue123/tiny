...
通过阅读注释得知，这个文件实现了symbol table(符号表)，数据结构是一个链式哈希表。个人感觉，这个具体的代码实现没必要看，反正到时候修改成TINY+时应该不会改这里面的代码。只用知道头文件里面定义的三个函数的用法就行了。
...
#### printSymTab
打印符号表。将main.c中的TraceAnalyze置为TRUE可以对符号表进行打印。
|Variable Name | Location  | Line Numbers|
|------------- | --------  | ------------|
|x             | 0           | 5    6    9   10   10   11|
|fact          | 1           | 7    9    9   12|
可以看出，符号表存储了变量名、变量在内存中分配的位置，变量出现的行数

### ANALYZE.H ANALYZE.C

#### traverse
这个函数有三个参数，树的根结点t和两个函数指针preProc和postProc。  
其中，preProc表示在对树执行先序遍历时对每个结点进行的操作。postProc表示在对树执行后序遍历时对每个结点进行的操作。实际使用时，通常将其中一个参数置为nullProc(什么也不做的函数)。  
在结束对一个根结点的遍历+操作之后，会再到这个根结点的下一个sibling进行同样的操作。这一过程对应于stmt-sequence中，完成一个statement的分析后，再进行下一个statement的分析。

#### insertNode
这个的作用是根据树结点的类型更新符号表。如果这个结点的类型为AssignK, ReadK或IdK，就将涉及到的变量插入符号表。然而在TINY+语言中规定，变量必须先声明后使用，因此这个地方到时候要改，应该是改成声明语句才能将变量插入符号表。

#### buildSymtab
作用是按照先序遍历的顺序对每个树结点进行insertNode的操作，这样就可以构建出符号表。这里为什么要用先序遍历呢？我试了一下，将函数中的insertNode和nullProc交换位置，改成后序遍历，得到的结果其实和先序遍历是一样的。所以我猜测应该也可以用后序遍历。  
不过还是按原来的用先序遍历吧。

#### checkNode
作用是对每个树结点进行类型检查。比如说运算符号的两个子结点必须是Integer类型，运算符号本身如果是+-*/那么也是Integer类型，如果是=<那么就是Boolean类型。如果结点类型是常数ConstK或者变量IdK，那么就赋值成Integer类型。

#### typeCheck
作用是按照后序遍历的顺序对每个树结点进行checkNode的操作。这个操作只有检查语义是否有错误的功能。这个只能用后序遍历，因为一开始这些结点的初值都是Void类型，如果先访问根结点的话，根结点要检查的子结点还没有被赋值，这样就肯定会报错。

### CODE和CGEN
这两个文件主要是用来生成类似汇编语言的，太难了，就不读了。